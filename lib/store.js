...
async function saveProducts(products) {
  await withTransaction(async (client) => {
    ...
    console.log("Products after delete:", count.rows[0].count);

    for (const p of products) {
      console.log("Insert:", p.id);

      await client.query(
        `INSERT INTO products
        (
          id,name,category,brand,series,character,
          price,old_price,status,description,
          images,description_images,featured,created_at
        )
        VALUES
        ...
        [
          ...
          JSON.stringify(p.descriptionImages || []),
          !!p.featured,
          p.createdAt || new Date().toISOString()
        ]
      );
      console.log("INSERT OK:", p.id);
    }
...
async function createProduct(product) {
  ...
  await db.query(
    ...
    [
      ...
      JSON.stringify(product.descriptionImages || []),
      !!product.featured,
...
// =====================================================
// PAYMENT SETTINGS (singleton config)
// Bank-transfer details shown on the checkout page's QR option.
// =====================================================
function defaultPaymentSettings() {
  return {
    bankName: 'Sacombank',
    accountName: 'CONG TY WFIGURE',
    accountNumber: '0699999999999',
    transferNote: 'Mã đơn hàng của bạn',
    qrImageUrl: null,
    updatedAt: null
  };
}
...
async function getPaymentSettings() {
  ...
  if (!rows.length) return defaultPaymentSettings();
  const r = rows[0];
  return {
...
async function savePaymentSettings(settings) {
  await db.query(
    `INSERT INTO payment_settings (id, bank_name, account_name, account_number, transfer_note, qr_image_url, updated_at)
     VALUES (1, $1,$2,$3,$4,$5,$6)
     ON CONFLICT (id) DO UPDATE SET
       bank_name = EXCLUDED.bank_name, account_name = EXCLUDED.account_name,
       account_number = EXCLUDED.account_number, transfer_note = EXCLUDED.transfer_note,
    ...
    [
      ...
      settings.transferNote || '', settings.qrImageUrl || null, settings.updatedAt || null
    ]
  );
}
